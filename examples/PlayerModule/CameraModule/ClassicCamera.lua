	...
	if subjectPosition and player and camera then
		...
		if self:GetIsMouseLocked() and not self:IsInFirstPerson() then
			...
			if not userPanningTheCamera and self.lastCameraTransform then
				...
				if (isInVehicle or (self.isFollowCamera and isClimbing)) and self.lastUpdate and humanoid and humanoid.Torso then
					if isInFirstPerson then
						if self.lastSubjectCFrame and (isInVehicle) and cameraSubject:IsA("BasePart") then
		...
		if isInVehicle and cameraSubject:IsA("BasePart") then
